...
\begin{itemize}
    ...
    \item{O trabalho desenvolvido nesta dissertação segue a linha de
        pesquisa utilizada por Dias and Dias~(2009) e nós apresentaremos
    ...
    \item{Nós fizemos comparações com os modelos de Programação por
        Restrições para ordenação por transposições, descrito por Dias and
        Dias~(2009), e no caso de ordenação por reversões e ordenação
        por reversões e transposições, com os modelos de Programação
        Linear Inteira (PLI/ILP) descritos por Dias and Souza~(2007).}
...
\begin{itemize} 
    ...
    \item{Melhor algoritmo de aproximação possui razão de $1.375$ e foi
        apresentado por Berman, Hannenhalli and Karpinski (2002).}
...
\begin{itemize}
    ...
    \item{Introduzido por Bafna and Pevzner (1998).}
...
\begin{itemize}
    ...
    \item{Seja $c_{\text{ímpar}}(\pi)$ o número de ciclos ímpares de
        ...
        $\rho$, temos que $\Delta c_{\text{ímpar}} \in \{2, 0, -2\}$ [Bafna and
        Pevzner (1998)].}
...
\begin{itemize}
    ...
    \item{Na natureza uma genoma não sofre apenas eventos de reversão ou
    ...
    \item{Hannenhalli, Chappey, Koonin and Pevzner (1995), Walter, Dias
        and Meidanis (1998, 2002), Gu, Peng and Sudborough (1999) e Lin
    and Xue (1999) estudaram este problema.} 
...
\begin{beamerboxesrounded}[center,shadow=true]{Permutação:} 
    ...
    ($\pi_{1},\pi_{2},\cdots,\pi_{n}$) onde $\pi_{i} \in \mathbb{N}$, $0
    < \pi_{i} \le n$ e $\pi_{i} \neq \pi_{j}$ para $i \neq j$. A
...
\begin{itemize} 
    ...
    \item{Para ILP usamos as formulações descritas em Dias and Souza
        (2007).}
...
\begin{beamerboxesrounded}[center,shadow=true]{Em ILP:}
    ...
    \begin{itemize}
        ...
        \item{ILOG CPLEX foi melhor nas permutações com $n \leq 7$.}

        \item{GLPK foi melhor nas permutações com $n > 7$, com excessão
            das permutações com $n = 9$.}
